...
# Dynamically detect workspace directory (where this file is located)
...
class AgentClient:
    ...
    def __init__(self):
        ...
        self.messages = []
        self.max_turns = 15  # Allow enough for testing and fixing errors
    ...
    def run_conversation(self, user_prompt: str, progress_callback=None):
        ...
        while turn_count < self.max_turns:
            ...
            if "tool_calls" in msg and len(msg["tool_calls"]) > 0:
                ...
                # Count successful actions (non-read operations)
                if event_type in ["WRITE_FILE", "CREATE_DIR"]:
                ...
                if event_type == "EXECUTE":
                    ...
                    has_error = any(err in result_str for err in ["error", "failed", "cannot find module", "module not found"])
                    if has_error:
                        error_history.append(str(result))
                        if len(error_history) >= 3:
                            # Check if errors are repeating
                            if len(set(error_history[-3:])) <= 1:  # Same error 3 times
                                steps.append(("ERROR", "Detected repeated errors. Stopping to prevent infinite loop."))
                                if progress_callback:
                                    progress_callback("ERROR", "Detected repeated errors. Stopping.")
                                break
                    else:
                        error_history.clear()  # Clear on successful command
                ...
                # For file creation subtasks, stop after 1 write action
                # For testing/fixing subtasks, allow multiple actions until completion
                if successful_actions >= 1 and event_type != "EXECUTE":
...
BAD examples:
❌ Create package.json (missing project folder!)
❌ Add "next" dependency to package.json (too granular - do all deps at once)
❌ Add import React to index.js (too granular - imports go with file creation)
❌ Create pages/index.js (missing project folder prefix!)
❌ Import from './Header' in pages/index.js (wrong path - should be '../components/Header')
...
Your subtasks:"""
        ...
        for i, subtask in enumerate(subtasks, 1):
            ...
            if progress_callback:
                progress_callback("SUBTASK_DONE", f"✓ Completed step {i}/{len(subtasks)}")
...
""")
    ...
    def _build_contextual_prompt(self, original_request, project_context, current_subtask,
        ...
        if project_context['project_name']:
            ...
            context_parts.append(f"  ✓ ALL files must start with '{proj_name}/'")
            context_parts.append(f"  ✓ Example: '{proj_name}/pages/index.js' (CORRECT)")
            context_parts.append(f"  ✗ Example: 'pages/index.js' (WRONG - missing {proj_name}/)")
            context_parts.append(f"  ✗ Example: 'components/File.js' (WRONG - missing {proj_name}/)")
            context_parts.append(f"\nOTHER RULES:")
            context_parts.append(f"  ✓ Do ONLY the current task above")
            context_parts.append(f"  ✗ DO NOT do future steps early")
            context_parts.append(f"  ✗ DO NOT create extra files not mentioned in current task")
...
DATA FLOW:
- User input (city name) → WeatherForm
- WeatherForm → API call → Weather data
- Weather data → WeatherDisplay component
